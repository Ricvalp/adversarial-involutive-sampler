...
class Trainer:
    ...
    def init_model(self):
        ...
        discriminator_params = discriminator.init(init_rng, jax.random.normal(init_points_rng, (100, 2 * self.cfg.kernel.d)))['params']
        ...
        discriminator_optimizer = optax.adam(learning_rate=self.cfg.train.discriminator_learning_rate)
        
        self.L_state = TrainState.create(apply_fn=discriminator.L.apply, params=theta_params, tx=L_optimizer)
        self.D_state = TrainState.create(apply_fn=discriminator.apply, params=phi_params, tx=discriminator_optimizer)
    ...
    def create_data_loader(self, key):
        ...
        samples = self.sample(
            ...
            name=f"samples_in_data_loader.png"
        )
    ...
    def train_epoch(self, epoch_idx):
        ...
        self.rng = self.create_data_loader(self.rng)
        for i, batch in enumerate(self.data_loader):
            ...
            for _ in range(self.cfg.train.num_adversarial_steps):
                self.D_state, adv_loss = self.minimize_adversarial_loss_step(self.L_state, self.D_state, batch)
