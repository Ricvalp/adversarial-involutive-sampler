...
def get_hamiltonian_density_image(density, xlim_q, ylim_q, xlim_p, ylim_p, q_0=0., q_1=1., n=100):
    ...
    z_q = jnp.concatenate(
        jnp.array([jnp.hstack([X_q.reshape(-1, 1), Y_q.reshape(-1, 1)]),
                   jnp.hstack([jnp.zeros((n**2, 1)), jnp.zeros((n**2, 1))])]),
                   axis=1
                   )
    z_p = jnp.concatenate(
        jnp.array([jnp.hstack([jnp.zeros((n**2, 1)) + q_0, jnp.zeros((n**2, 1)) + q_1]),
...
def plot_samples_with_momentum(samples, target_density, q_0=0., q_1=0.,  name=None, **kwargs):
    ...
    Z_p, Z_q = get_hamiltonian_density_image(target_density, xlim_q, ylim_q, xlim_p, ylim_p, q_0=q_0, q_1=q_1, n=100)
